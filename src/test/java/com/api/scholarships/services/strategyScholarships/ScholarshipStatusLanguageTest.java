...
@ActiveProfiles("test")
class ScholarshipStatusLanguageTest {
  ...
  @InjectMocks
  private ScholarshipStatusLanguage scholarshipStatusLanguage;
  ...
  @Test
  @DisplayName("Test ScholarshipStatusLanguage, test to find scholarships by language and status")
  void testToFindScholarshipsByLanguageAndStatus() {
    //given
    given(statusService.findByName(Variables.STATUS_DEFAULT)).willReturn(status);
    given(languageService.findById(anyLong())).willReturn(language);
    ...
    given(scholarshipRepository.findByStatusAndLanguage(status,language,pageable)).willReturn(scholarships);
    //when
    Page<Scholarship> scholarshipsFound=scholarshipStatusLanguage.findScholarshipsByCondition(pageable,1L);
    //then
    assertAll(
